...
# STATE CALLBACK FUNCTION: This forces data queries to finalize completely before redrawing the dashboard layout frames
def execute_slate_optimization_callback():
    ...
    for game in games_found:
        ...
        ml_market_prob = np.random.uniform(0.45, 0.55) # Derived clean market line (pre-juice)
...
if st.session_state.trading_slate_calculated:
    ...
    for idx, bet in enumerate(st.session_state.cached_optimized_wagers):
        ...
        if wager_amt > 0.01:
            ...
            with st.container():
                ...
                # INTEGRATED PROBABILITY PANEL FEED
    ...
    st.write(f"Actual Bankroll Exposure: **{ (running_total_liability / bankroll) * 100:.2f}%** out of a maximum {daily_max_exposure_pct}.00%")import streamlit as st
import numpy as np
import torch
import torch.nn as nn
from torch.utils.data import DataLoader, TensorDataset
